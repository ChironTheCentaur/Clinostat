...
class gravitySystem:
    ...
    def __init__(self):
        ...
        self.scaleFactor = 1    #determines how agressive the 
    ...
    def gravityRun(self):
        ...
        logFile.write("Loop#, Current X, Current Y, Current Z, Gravity\n")
        accel = dict()

        #takes the input gravity and sets the boundry conditions propperly
        xMin = -1 + 0*self.approachPercent
        xMax = 1 - 0*self.approachPercent
        yMin = -1 + 0*self.approachPercent
        yMax = 1 - 0*self.approachPercent
        zMin = self.target - self.approachPercent
        zMax = self.target + self.approachPercent
        ...
        while (float(time.time() - startTime) < self.runTime )and self.shutdown == False:
            ...
            try:
                ...
                logFile.write(f"{loop}, {accel['x']}, {accel['y']}, {accel['z']}, {self.gAvg}\n")
            ...
            if(self.target!= -1 and (xMin <= accel['x'] <= xMax) and (yMin <= accel['y'] <= yMax) and (zMin <= accel['z'] <= zMax)):
                mAdj = 1*(self.scaleFactor*abs(self.target - self.gAvg))
                print(mAdj)
            else:
                mAdj = 1
            ...
            if(loopDirChange > 3000): #will attempt to change the direction every 30 seconds
            ...
            loopDirChange += 1
            totLoop += 1
            time.sleep(0.01)  #loops every tenth of a second
